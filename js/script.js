...
function initCountdowns() {
  ...
  if (document.getElementById('hours') && document.getElementById('minutes') && document.getElementById('seconds')) {
    startCountdown('hours', 'minutes', 'seconds', 24 * 60 * 60); // 24 hours in seconds
  }
  
  // Landing page footer countdown (sync with main)
  if (document.getElementById('hours-footer') && document.getElementById('minutes-footer') && document.getElementById('seconds-footer')) {
    startCountdown('hours-footer', 'minutes-footer', 'seconds-footer', 24 * 60 * 60); // 24 hours in seconds
  }
  
  // Upsell page countdown (15 minutes)
  if (document.getElementById('upsell-minutes') && document.getElementById('upsell-seconds')) {
    startMinuteCountdown('upsell-minutes', 'upsell-seconds', 15); // 15 minutes
  }
  
  // Upsell page footer countdown (sync with main)
  if (document.getElementById('upsell-minutes-footer') && document.getElementById('upsell-seconds-footer')) {
    startMinuteCountdown('upsell-minutes-footer', 'upsell-seconds-footer', 15); // 15 minutes
  }
  
  // Downsell page countdown (10 minutes)
  if (document.getElementById('downsell-minutes') && document.getElementById('downsell-seconds')) {
    startMinuteCountdown('downsell-minutes', 'downsell-seconds', 10); // 10 minutes
  }
  
  // Downsell page footer countdown (sync with main)
  if (document.getElementById('downsell-minutes-footer') && document.getElementById('downsell-seconds-footer')) {
    startMinuteCountdown('downsell-minutes-footer', 'downsell-seconds-footer', 10); // 10 minutes
  }
  
  // Checkout page countdown (15 minutes)
  if (document.getElementById('checkout-minutes') && document.getElementById('checkout-seconds')) {
    startMinuteCountdown('checkout-minutes', 'checkout-seconds', 15); // 15 minutes
  }
}
...
function startMinuteCountdown(minutesId, secondsId, totalMinutes) {
  ...
  let timer = totalMinutes * 60; // Convert minutes to seconds
...
function initScrollAnimations() {
  const elements = document.querySelectorAll('.benefit-card, .feature-card, .testimonial-card, .pricing-card, .upsell-feature, .downsell-offer');
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const pulseElements = document.querySelectorAll('.highlight, .countdown, .pricing-value, .guarantee-badge');
  ...
  // Add local storage for user data persistence
  initLocalStorage();
});
...
function initLocalStorage() {
  ...
  if (!hasVisited) {
    ...
    // Returning visitor - personalize experience
  ...
  forms.forEach(form => {
    ...
    inputs.forEach(input => {
      ...
      input.addEventListener('change', function() {
        ...
        // If this is a name field, save it specifically
